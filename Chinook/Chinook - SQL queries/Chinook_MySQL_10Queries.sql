...
select ROW_NUMBER() OVER(ORDER BY (SUM(il.Quantity*il.UnitPrice))desc) as 'Rank' ,CONCAT(c.FirstName,',',LastName) as NAME, SUM(il.Quantity*il.UnitPrice ) as TotalSales
from Invoice i 
join InvoiceLine il on il.InvoiceId=i.InvoiceId
join Customer c on c.CustomerId=i.CustomerId 
