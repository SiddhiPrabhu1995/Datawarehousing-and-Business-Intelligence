...
FROM Sales.SalesOrderDetail sod
 JOIN Sales.SalesOrderHeader sh ON sh.SalesOrderID=sod.SalesOrderID
 JOIN Sales.SpecialOfferProduct sp ON sp.SpecialOfferID = sod.SpecialOfferID
AND sp.ProductID=sod.ProductID
 JOIN Production.ProductProductPhoto pp ON pp.ProductID = sp.ProductID
 JOIN Production.Product p ON p.ProductID = pp.ProductID
...
from Sales.SalesOrderDetail sod 
 JOIN Sales.SalesOrderHeader sh ON sh.SalesOrderID=sod.SalesOrderID
 JOIN Sales.SpecialOfferProduct sp ON sp.SpecialOfferID = sod.SpecialOfferID AND sp.ProductID=sod.ProductID
 JOIN Production.ProductProductPhoto pp ON pp.ProductID = sp.ProductID
 JOIN Production.Product p ON p.ProductID = pp.ProductID
 JOIN Production.ProductSubcategory ps ON ps.ProductSubcategoryID = p.ProductSubcategoryID
 JOIN Production.ProductCategory pc ON pc.ProductCategoryID = ps.ProductCategoryID
...
from Sales.SalesOrderDetail sod 
 ...
 JOIN Production.ProductSubcategory ps ON ps.ProductSubcategoryID = p.ProductSubcategoryID
 JOIN Production.ProductCategory pc ON pc.ProductCategoryID = ps.ProductCategoryID
 JOIN Sales.SalesTerritory t ON t.TerritoryID=sh.TerritoryID
 JOIN Person.CountryRegion cr ON t.CountryRegionCode=cr.CountryRegionCode
WHERE sh.OnlineOrderFlag IN (0,1)
GROUP BY  cr.Name;
...
from Sales.SalesOrderDetail sod 
 ...
 JOIN Sales.SalesTerritory t ON t.TerritoryID=sh.TerritoryID
 JOIN Person.CountryRegion cr ON t.CountryRegionCode=cr.CountryRegionCode
 JOIN Person.StateProvince state ON state.CountryRegionCode=cr.CountryRegionCode
 JOIN Person.Address a ON a.StateProvinceID= state.StateProvinceID
...
FROM Sales.SalesOrderDetail sod
 ...
 JOIN Sales.vStoreWithDemographics vdemo ON vdemo.BusinessEntityID=s.BusinessEntityID
...
FROM Sales.SalesOrderDetail sod
 JOIN Sales.SalesOrderHeader soh ON soh.SalesOrderID=sod.SalesOrderID
 JOIN Sales.Customer c ON c.CustomerId=soh.CustomerId
 JOIN Sales.vIndividualCustomer vdemo ON vdemo.BusinessEntityID=c.CustomerId
...
from Sales.SalesOrderDetail sod
 ...
 JOIN Sales.Customer c ON c.CustomerId=soh.CustomerId
 JOIN Sales.SalesPerson s ON s.TerritoryID=soh.TerritoryID
 JOIN Person.Person per ON per.BusinessEntityID=c.PersonID
 JOIN Sales.vPersonDemographics vdemo ON vdemo.BusinessEntityID=per.BusinessEntityID
...
FROM Sales.SalesOrderDetail sod
 ...
 JOIN Sales.SpecialOfferProduct sop ON sop.ProductID=sod.ProductID AND sop.ProductID=sod.ProductID
 JOIN Sales.SpecialOffer specialop ON specialop.SpecialOfferID=sop.SpecialOfferID 
 JOIN Production.ProductProductPhoto as ppp ON ppp.ProductID=sop.ProductID
 JOIN Production.Product as prod ON prod.ProductID = sop.ProductID
 JOIN Production.ProductSubcategory AS prodsub ON prodsub.ProductSubcategoryID=prod.ProductSubcategoryID
 JOIN Production.ProductCategory as prodcat ON prodcat.ProductCategoryID=prodsub.ProductCategoryID
WHERE soh.OnlineOrderFlag='0'
...
from Sales.SalesOrderDetail sod 
 JOIN Sales.SpecialOfferProduct sp ON sp.SpecialOfferID = sod.SpecialOfferID AND sp.ProductID=sod.ProductID
 JOIN Production.ProductProductPhoto pp ON PP.ProductID = sod.ProductID
 JOIN Production.Product p ON p.ProductID = pp.ProductID
 JOIN Production.WorkOrder wo ON wo.ProductID = p.ProductID
 JOIN Sales.SalesOrderHeader sh ON sh.SalesOrderID=sod.SalesOrderID
 JOIN Sales.SalesTerritory t ON t.TerritoryID=sh.TerritoryID
 JOIN Person.CountryRegion cr ON t.CountryRegionCode=cr.CountryRegionCode
WHERE sh.OnlineOrderFlag IN (0,1)
