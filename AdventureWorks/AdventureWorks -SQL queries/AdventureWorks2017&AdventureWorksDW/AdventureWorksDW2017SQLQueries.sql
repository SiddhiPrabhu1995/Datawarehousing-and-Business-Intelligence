...
from FactInternetSales fis 
 ...
 JOIN DimProductSubcategory subcat ON subcat.ProductSubcategoryKey = dp.ProductSubcategoryKey
 JOIN DimProductCategory prodcat ON prodcat.ProductCategoryKey = subcat.ProductCategoryKey
...
from cte
 JOIN DimProduct dp ON dp.ProductKey=cte.ProductKey
 JOIN DimProductSubcategory subcat ON subcat.ProductSubcategoryKey = dp.ProductSubcategoryKey
 JOIN DimProductCategory prodcat ON prodcat.ProductCategoryKey = subcat.ProductCategoryKey
GROUP BY prodcat.EnglishProductCategoryName;
...
from cte
 JOIN DimProduct dp ON dp.ProductKey=cte.ProductKey
 JOIN DimSalesTerritory st ON st.SalesTerritoryKey=cte.SalesTerritoryKey
...
from cte
 JOIN DimProduct dp ON dp.ProductKey=cte.ProductKey
 JOIN DimSalesTerritory st ON st.SalesTerritoryKey=cte.SalesTerritoryKey
 JOIN DimGeography dg ON dg.SalesTerritoryKey=st.SalesTerritoryKey 
...
SELECT top 10 dimre.BusinessType,
           ...
           RANK() OVER(ORDER BY SUM(fis.UnitPrice*fis.OrderQuantity) DESC) AS 'Rank'
FROM FactResellerSales fis
 JOIN DimReseller dimre ON dimre.ResellerKey=fis.ResellerKey
...
SELECT fis.EmployeeKey,
       ...
       RANK() OVER(ORDER BY SUM(fis.UnitPrice*fis.OrderQuantity) DESC) AS 'Rank'
FROM FactResellerSales fis
 JOIN DimEmployee e ON e.EmployeeKey=fis.EmployeeKey
GROUP BY e.LastName,
         e.FirstName,
         fis.EmployeeKey;
...
SELECT 	dimprom.EnglishPromotionCategory, dimprom.EnglishPromotionType, dimprom.EnglishPromotionName,
		...
		(SUM(fis.UnitPrice*fis.OrderQuantity)-SUM(fis.TotalProductCost))  AS 'Profit',
		SUM((fis.UnitPrice*fis.OrderQuantity)/100) AS 'Promotion %',
		RANK() OVER(ORDER BY SUM(fis.UnitPrice*fis.OrderQuantity) DESC) AS 'Rank'
from FactResellerSales fis 
 JOIN DimProduct dp ON dp.ProductKey=fis.ProductKey
 JOIN DimPromotion dimprom ON dimprom.PromotionKey = fis.PromotionKey
...
SELECT 	st.SalesTerritoryGroup, st.SalesTerritoryCountry, st.SalesTerritoryRegion,
		SUM(cte.UnitPrice*cte.OrderQuantity) Sales,
		SUM(cte.ProductStandardCost) ProductCosts,
		(SUM(cte.UnitPrice*cte.OrderQuantity)-SUM(cte.TotalProductCost))  AS 'Profit',
		COUNT(cte.SalesOrderNumber) AS 'Number of Orders',
		SUM(cte.OrderQuantity) AS 'Quantity ordered',
		RANK() OVER(ORDER BY SUM(cte.UnitPrice*cte.OrderQuantity) DESC) AS 'Rank'
from cte
 JOIN DimProduct dp ON dp.ProductKey=cte.ProductKey
 JOIN DimSalesTerritory st on cte.SalesTerritoryKey = st.SalesTerritoryKey
...
FROM dbo.FactInternetSales fis
 JOIN dbo.FactResellerSales frs ON frs.ProductKey=fis.ProductKey
 JOIN DimProduct dp ON dp.ProductKey=fis.ProductKey
